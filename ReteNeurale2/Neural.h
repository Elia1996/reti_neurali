...
#ifndef RETENEURALE2_NEURAL_H
#define RETENEURALE2_NEURAL_H
...
public:
    ...
    // ritorna
    T back_propagation(vector<T> in, vector<T> out){
        if(in.size()!=_n_in || out.size()!=_n_out)
            error("back_propagation: vettori sbagliati"), assert(in.size()!=_n_in || out.size()!=_n_out);
        // eseguo la rete sull'esempio
        run_rete(in);
        // PARAMETRI CHE USO
            vector<vector<vector<T>>> dw;
            Strato<T> *S1 = &_rete[_n_strati-1];
            Strato<T> *S2 = &_rete[_n_strati-2];
            for(int i=0;i<_n_strati; i++)
                dw.push_back(_rete[i].getDw());
            T err=0;
            T dE_dy;
            vector<T> sigma_j(_n_out);
            vector<T> sigma_k(S2->size());

        // ciclo sull'ultimo strato
        T delta=0, delta2=0;
        for(int j=0; j<_n_out ; j++){
            T j_out = ((*S1).getOut())[j];  // output del j-esimo percettrone esterno
            dE_dy = -(out[j] - j_out);   // -(dj-yj)
            ...
            int k;
            T delta=0, delta2=0;
            // CORREZIONE DI TUTTI I PESI
            // il più uno è stato messo perchè è stato fatto in modo nella classe Strato che ci fosse un percettrone in più che funge da BIAS
            for(k=0; k<_rete[_n_strati-2].size()+1; k++){
                ...
                S1->setDw( j, k, delta + delta2 );  // SALVO Il DELTA W  per il passaggio successivo
                S1->inc_peso( j, k, delta + delta2 );  // INCREMENTO IL PESO JK applicando la correzione deltaw trovata
            }
        }
        ...
        for(int k=0; k<_rete[0].size(); k++){
            ...
            int i;
            for(i=0; i<_n_in; i++){
                _rete[0].setDw(k,i, -_eta*_sigma_strato[0][k]*in[i]+ _eta2*dw[0][k][i]);
                _rete[0].inc_peso(   k,i, -_eta*_sigma_strato[0][k]*in[i]+ _eta2*dw[0][k][i] );
            }
    ...
    T training(vector<vector<T>>& in, vector<vector<T>>& out, int max_ere, double max_error, bool print, bool printRete=false, bool allError=false){
        T d_err;
        T error=10;
        ...
        if(print)
            cout << "---------- training ------------" << endl;
        for(i=0; i<max_ere && error>max_error ; i++ ){
            //cout << "################### ERA " << i << " ############################" << endl;
            error=0;
            // ciclo sugli esempi che ho e correggo i pesi
            for(int j=0; j<in.size(); j++){
                // eseguo la backpropagation
                error+=this->back_propagation(in[j],out[j]);
                // eseguo la rete
                this->run_rete(in[j]);
            }
            if(print && (allError || error<(1/zeri)))
                cout << "error: " <<error/4 << endl, zeri*=10;
    ...
    T training(vector<vector<T>>& in, vector<vector<T>>& out, bool print, bool printRete=false,bool allError=false){
        T d_err;
        T error=10;
        // ere, ogni era prova tutti gli esempi e corregge i pesi, questo viene ripetuto a ogni era
        int i;
        ...
        if(print)
            cout << "---------- training ------------" << endl;
        for(i=0; i<_max_ere && error>_max_error ; i++ ){
            //cout << "################### ERA " << i << " ############################" << endl;
            error=0;
            // ciclo sugli esempi che ho e correggo i pesi
            for(int j=0; j<in.size(); j++){
    ...
    T training( bool print, bool printRete=false, bool allError=false){
        ...
        for(i=0; i<_max_ere && error>_max_error ; i++, iPrint++ ){
            ...
            for(int j=0; j<_in.size(); j++){
                // eseguo la backpropagation
                error+=this->back_propagation(_in[j],_out[j]);
                // eseguo la rete
                this->run_rete(_in[j]);
            }
